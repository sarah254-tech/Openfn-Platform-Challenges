...
var decode = (base64Data, options2 = { parseJson: true }) => {
  ...
  if ((import_lodash.default.startsWith(decodedValue, "[") || import_lodash.default.startsWith(decodedValue, "{")) && options2.parseJson) {
    try {
...
function jsonValue(obj, path2) {
  return (0, import_jsonpath_plus.JSONPath)({ path: path2, json: obj })[0];
}
function sourceValue(path2) {
  return (state) => {
    return (0, import_jsonpath_plus.JSONPath)({ path: path2, json: state })[0];
  };
}
function source(path2) {
  return (state) => {
    return (0, import_jsonpath_plus.JSONPath)({ path: path2, json: state });
  };
}
...
var getParser = (csvData, options2) => {
  if (typeof csvData === "string") {
    return (0, import_csv_parse.parse)(csvData, options2);
  }
  let stream = csvData;
  if (csvData instanceof ReadableStream) {
    stream = import_node_stream3.Readable.from(csvData);
  }
  return stream.pipe((0, import_csv_parse.parse)(options2));
};
...
function validate(schema = "schema", data = "data") {
  return async (state) => {
      ...
      if (typeof schemaOrUrl === "string") {
        try {
          ...
          if (e instanceof TypeError) {
            return (0, import_jsonpath_plus.JSONPath)({ path: schemaOrUrl, json: state })[0];
          } else {
    ...
    function resolveData() {
      ...
      if (typeof d === "string") {
        return (0, import_jsonpath_plus.JSONPath)({ path: d, json: state })[0];
      }
      return d;
    }
  };
...
var helpers = {
  json: function() {
    (0, import_set.default)(this, "headers.Content-Type", "application/json");
    return this;
  },
  basic: function(username, password) {
    const buff = Buffer.from(`${username}:${password}`);
    const credentials = buff.toString("base64");
    (0, import_set.default)(this, "headers.Authorization", `Basic ${credentials}`);
    return this;
  },
  bearer: function(token) {
    (0, import_set.default)(this, "headers.Authorization", `Bearer ${token}`);
    return this;
  },
